module Vibebrew
  module Saas
    class Subscription < ApplicationRecord
      ...
      validates :plan_name, presence: true, inclusion: { in: Vibebrew::Saas::Plan::TIERS.keys.map(&:to_s) }

      scope :active_or_trialing, -> { where(status: [ :active, :trialing ]) }

      def plan
        Vibebrew::Saas::Plan.new(plan_name)
      end

      def plan=(new_plan)
        self.plan_name = new_plan.is_a?(Vibebrew::Saas::Plan) ? new_plan.name.to_s : new_plan.to_s
      end
